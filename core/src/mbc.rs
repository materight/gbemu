...
impl MBC1 {
    ...
    fn ram_addr(&self, addr:u16, ram: &[u8]) -> usize {
        ...
        else { bank_addr(addr, self.ram_bank as u16, 0xA000, 0x2000) }
    }

    fn high_bank(&self, rom_size: usize, zero: bool) -> u8 {
        let mut bank_nr = if !zero { mask_bank_nr(self.rom_bank as u16, rom_size) as u8 } else { 0 };
...
impl MBCType for MBC1 {
    ...
    fn r(&self, addr: u16, rom: &[u8], ram: &[u8]) -> u8 { 
        match addr {
            0x0000..=0x3FFF => if self.mode { rom[bank_addr(addr, self.high_bank(rom.len(), true) as u16, 0, 0x4000)] } else { rom[addr as usize] },
    ...
    fn w(&mut self, addr: u16, val: u8, rom: &[u8], ram: &mut [u8]) {
        match addr {
            ...
            0x2000..=0x3FFF => self.rom_bank = if val & 0x1F != 0 { mask_bank_nr(val as u16, rom.len()) as u8 } else { 1 },
...
impl MBCType for MBC3 {
    ...
    fn r(&self, addr: u16, rom: &[u8], ram: &[u8]) -> u8 { 
        match addr {
            ...
            0xA000..=0xBFFF => 
                ...
                else                    { ram[bank_addr(addr, self.ram_bank as u16, 0xA000, 0x2000)] },
            _ => 0x00,
        }
    }
    ...
    fn w(&mut self, addr: u16, val: u8, _: &[u8], ram: &mut [u8]) {
        match addr {
            ...
            0xA000..=0xBFFF => if self.ram_enabled { ram[bank_addr(addr, self.ram_bank as u16, 0xA000, 0x2000)] = val },
            _ => ()
        }
    }
}
...
impl MBCType for MBC5 {
    ...
    fn r(&self, addr: u16, rom: &[u8], ram: &[u8]) -> u8 { 
        match addr {
            ...
            0xA000..=0xBFFF => if self.ram_enabled { ram[bank_addr(addr, self.ram_bank as u16, 0xA000, 0x2000)] } else { 0xFF },
            _ => 0x00,
        }
    }
