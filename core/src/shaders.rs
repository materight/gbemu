        ...
        for y in 0..LCDH {
            ...
            for dx in 0..scale {
                for dy in 0..scale {
                    ...
                    out[idx..idx + 4].copy_from_slice(&[r, g, b, 0xFF]);
                }
            }
        }
    }
        ...
        for y in 0..LCDH {
            ...
            for dx in 0..scale {
                for dy in 0..scale {
                    ...
                    out[idx..idx + 4].copy_from_slice(&[rl, gr, br, 0xFF]);
                }
            }
        }
    }
            ...
            for dx in 0..scale {
                for dy in 0..scale {
                    ...
                    if scale >= 2 && dy == scale - 1 {
                        // Draw scanlines
                        out[idx..idx + 4].copy_from_slice(&[r / 3, g / 3, b / 3, 0xFF]);
                    } else if scale >= 3 && dx == 0 {
                        // Draw left chromatic aberration
                        out[idx..idx + 4].copy_from_slice(&[r.saturating_add(0x30), g, b, 0xFF]);
                    } else if scale >= 3 && dx == scale - 1 {
                        // Draw right chromatic aberration
                        out[idx..idx + 4].copy_from_slice(&[r, g, b.saturating_add(0x30), 0xFF]);
                    } else {
                        // Draw normal pixel
                        out[idx..idx + 4].copy_from_slice(&[r, g, b, 0xFF]);
                    }
